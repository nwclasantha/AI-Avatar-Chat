...
class DiscriminatorFullModel(torch.nn.Module):
    ...
    def get_zero_tensor(self, input):
        if self.zero_tensor is None or self.zero_tensor.device != input.device:
            self.zero_tensor = torch.zeros(1, device=input.device, requires_grad=False)
        return self.zero_tensor.expand_as(input)

    def to(self, *args, **kwargs):
        result = super().to(*args, **kwargs)
        self.pyramid = self.pyramid.to(*args, **kwargs)
        self.zero_tensor = None  # reset so it gets recreated on the right device
        return result
