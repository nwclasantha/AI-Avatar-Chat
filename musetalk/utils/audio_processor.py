...
class AudioProcessor:
    ...
    def get_whisper_chunk(
        ...
        if not whisper_input_features:
            raise ValueError("No audio features were produced for the provided audio input.")
        whisper_feature = []
        ...
        for frame_index in range(num_frames):
            try:
                ...
                raise RuntimeError(
                    f"Audio feature alignment error at frame {frame_index}: {e}\n"
                    f"whisper_feature.shape: {whisper_feature.shape}, "
                    f"num frames: {num_frames}, fps: {fps}, "
                    f"whisper_idx_multiplier: {whisper_idx_multiplier}, "
                    f"audio_index: {audio_index}-{audio_index + audio_feature_length_per_frame}"
                ) from e
...
if __name__ == "__main__":
    ...
    wav_path = "./2.wav"
    audio_features, librosa_feature_length = audio_processor.get_audio_feature(wav_path)
    print("Audio Feature shapes:", [f.shape for f in audio_features])
