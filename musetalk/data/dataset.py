...
class FaceDataset(Dataset):
    ...
    def __init__(self,
        ...
        for list_path, repeat_time in zip(list_paths, repeats):
            with open(list_path, 'r') as f:
                ...
                for line in f.readlines():
                    ...
                    if not line_info or line_info.startswith("#"):
                        continue
        ...
        if not self.meta_paths:
            raise ValueError(f"No metadata files were loaded from: {list_paths}")
    ...
    def get_audio_file(self, wav_path, start_index):
        ...
        audio_input = audio_input_librosa
    ...
    def __getitem__(self, idx):
        ...
        while attempts < self.max_attempts:
            ...
            if not meta_data.get("isvalid", True):
                attempts += 1
                print(f"video {video_path} is marked invalid in metadata")
                continue
            ...
            min_face_span = min(min(face_shape) for face_shape in face_shapes)
            if self.contorl_face_min_size and min_face_span < self.min_face_size:
                print(f"video {video_path} has face size {min_face_span} less than minimum required {self.min_face_size}")
                attempts += 1
                continue
            ...
            for drive_idx in drive_idx_list:
                ...
                if np.count_nonzero(np.asarray(face_mask)) == 0:
...
class HDTFDataset(FaceDataset):
    ...
    def __init__(self, cfg, split="train"):
        ...
        list_path = f'./dataset/HDTF/{split}.txt'
        if not os.path.exists(list_path):
            raise FileNotFoundError(f"Dataset split file not found: {list_path}")
...
class VFHQDataset(FaceDataset):
    ...
    def __init__(self, cfg, split="train"):
        ...
        list_path = f'./dataset/VFHQ/{split}.txt'
        if not os.path.exists(list_path):
            list_path = './dataset/VFHQ/train.txt'
        list_paths = [list_path]
        repeats = [1]
        super().__init__(cfg, list_paths, root_path, repeats)
        print('VFHQDataset: ', len(self))
        
def PortraitDataset(cfg=None, split="train"):
    ...
    if cfg["dataset_key"] == "HDTF":
        return ConcatDataset([HDTFDataset(cfg, split=split)])
    elif cfg["dataset_key"] == "VFHQ":
        return ConcatDataset([VFHQDataset(cfg, split=split)])
    else:  
        print("############ use all dataset ############ ")
        return ConcatDataset([HDTFDataset(cfg, split=split), VFHQDataset(cfg, split=split)])
