...
def debug_inpainting(video_path, bbox_shift, extra_margin=10, parsing_mode="jaw", 
    ...
    validate_media_inputs(video_path=video_path)
    ...
    if get_file_type(video_path) == "video":
        ...
        if first_frame is None:
            raise gr.Error(f"Unable to read input image: {video_path}")
    ...
    if not coord_list or not frame_list:
        raise gr.Error("Unable to load the first frame for debugging.")
    bbox = coord_list[0]
    frame = frame_list[0]
    ...
    if x2 <= x1 or y2 <= y1:
        raise gr.Error("The detected face crop is invalid for the selected parameters.")
    ...
    # Save results — get_image returns BGR (PIL RGB converted via [:,:,::-1])
    ...
    return combine_frame[:, :, ::-1], info_text
...
def validate_media_inputs(audio_path=None, video_path=None):
    if audio_path is not None and (not isinstance(audio_path, str) or not os.path.exists(audio_path)):
        raise gr.Error("Please provide a valid driving audio file.")
    if video_path is not None and (not isinstance(video_path, str) or not os.path.exists(video_path)):
        raise gr.Error("Please provide a valid reference video or image.")


def build_face_source_cycle(coord_list, frame_list, extra_margin):
    input_latent_list = []
    valid_frame_indices = []
    ...
    if not input_latent_list:
        raise gr.Error("No face was detected in the reference input. Try another frame or adjust the crop parameters.")

    input_latent_list_cycle = input_latent_list + input_latent_list[::-1]
    valid_frame_indices_cycle = valid_frame_indices + valid_frame_indices[::-1]
    return input_latent_list_cycle, valid_frame_indices_cycle
...
def inference(audio_path, video_path, bbox_shift, extra_margin=10, parsing_mode="jaw", 
    ...
    validate_media_inputs(audio_path=audio_path, video_path=video_path)
    ...
    if not input_img_list:
        raise gr.Error(f"No input frames were found for: {video_path}")
    ...
    if audio_result is None:
        raise gr.Error(f"Unable to load audio features from: {audio_path}")
    ...
    if len(whisper_chunks) == 0:
        raise gr.Error(f"No whisper chunks were generated for audio: {audio_path}")
    ...
    input_latent_list_cycle, valid_frame_indices_cycle = build_face_source_cycle(
        coord_list,
        frame_list,
        args.extra_margin,
    )
    ...
    # Build cycled latent list for datagen (latents are small tensors, acceptable to duplicate)
    gen = datagen(
    ...
    ############################################## inference + blend + write incrementally ##############################################
    print("start inference and blending")
    frame_idx = 0
    for i, (whisper_batch,latent_batch) in enumerate(tqdm(gen,total=int(np.ceil(float(video_num)/batch_size)))):
        ...
        # Blend and write each frame immediately instead of accumulating
        for res_frame in recon:
            source_idx = valid_frame_indices_cycle[frame_idx % len(valid_frame_indices_cycle)]
    ...
    if frame_idx == 0:
        raise gr.Error("Inference did not generate any output frames.")

    # Output video path with unique name to avoid collisions
    output_video = os.path.join(args.result_dir, f'temp_{uuid.uuid4().hex[:8]}.mp4')
    ...
    # Save video
    if not images:
        raise gr.Error("No rendered frames were written to disk.")
    imageio.mimwrite(output_video, images, 'FFMPEG', fps=fps, codec='libx264', pixelformat='yuv420p')

    # Check if the output_video and audio_path exist
    ...
    reader.close() # Otherwise, error on win11: PermissionError: [WinError 32]
    # Store frames in list
    frames = images

    print(len(frames))

    # Load the video and audio, combine, and write output
...
def check_video(video):
    ...
    try:
        ...
        reader.close()
    if fps <= 0:
        raise gr.Error("The uploaded video has an invalid frame rate.")
    target_fps = 25
    if not frames:
        raise gr.Error("The uploaded video does not contain any readable frames.")
    
    L = len(frames)
    L_target = max(1, int(round(L / fps * target_fps)))
    original_t = [x / fps for x in range(1, L+1)]
    t_idx = 0
    target_frames = []
    for target_t in range(1, L_target+1):
        while t_idx < L - 1 and target_t / target_fps > original_t[t_idx]:
