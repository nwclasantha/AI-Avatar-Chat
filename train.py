    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(dataloader_dict['train_dataloader']):
            ...
            with torch.no_grad():
                ...
                pixel_values_face_mask = batch['pixel_values_face_mask'].to(
                    ...
                    if cfg.use_adapted_weight:
                        ...
                        if image_audio_sim_gt < 0.05 or image_audio_sim_gt > 0.65:
                            if cfg.adapted_weight_type == "cut_off":
                                ...
                                adapted_weight = 1
                    adapted_weight_accum += float(adapted_weight)
            ...
            if cfg.loss_params.mouth_gan_loss > 0:
                ...
                if frames_mouth is not None and image_pred_mouth is not None:
                    pyramide_real_mouth = pyramid(downsampler(frames_mouth))
                    pyramide_generated_mouth = pyramid(downsampler(image_pred_mouth))
                    mouth_gan_ready = True
                else:
                    pyramide_real_mouth = None
                    pyramide_generated_mouth = None

            # Compute image pyramids for VGG and/or GAN loss
            if cfg.loss_params.vgg_loss > 0 or cfg.loss_params.gan_loss > 0:
            ...
            # Process mouth GAN loss if enabled
            if cfg.loss_params.mouth_gan_loss > 0 and mouth_gan_ready:
            ...
            # Backward pass
            avg_loss = accelerator.gather(loss.detach().repeat(bsz)).mean()
            ...
            if cfg.loss_params.gan_loss > 0:
                ...
                avg_loss_D = accelerator.gather(loss_D.detach().repeat(bsz)).mean()
            ...
            if cfg.loss_params.mouth_gan_loss > 0 and mouth_gan_ready:
                ...
                avg_mouth_loss_D = accelerator.gather(
                    mouth_loss_D.detach().repeat(frames_mouth.shape[0])).mean()
        ...
        if (epoch + 1) % cfg.save_model_epoch_interval == 0:
            try:
                ...
                if accelerator.is_main_process:
                    ...
                    epoch_save_path = os.path.join(save_dir, f"checkpoint-{global_step}")
